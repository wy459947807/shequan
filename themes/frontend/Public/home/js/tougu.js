...
;(function($){
    /*把对象调整到中心位置
     * 这里不用fixed定位，目的是让用户能滚动浏览其他内容，而不被弹窗遮挡
     */
...
;(function($) {
    ...
    $.tougu.photo_slide = function() {
        ...
        if (! (f != 1 && f < 4)) {
            ...
            if (!$.lojrw.util.empty(a) && !$.lojrw.util.empty(a.listID) && !$.lojrw.util.empty(a.listBtnID) && !$.lojrw.util.empty(a.feedClassName) && !$.lojrw.util.empty(a.feedWidth) && $("#" + a.listID).size() != 0) {
                var e = 0,
                    d = 1,
                    j = null,
                    h = [],
                    g = null,
                    g = $.lojrw.util.empty(a.feedBoxClass) ? $("#" + a.listID + " ul") : $("#" + a.listID + " ." + a.feedBoxClass),
                    k = function() {
    ...
    $.fn.uploader = function(options) {
        var settings = {
            ...
            showMessage: function(message){
                $.lojrw.tip({content:message, icon:'error'});
            },
            ...
            onComplete: function(id, fileName, result){
                if(result.status == '1'){
                    ...
                    $.lojrw.tip({content:result.msg, icon:'error'});
                }
            }
        };
        if(options) {
