...
class AdvertModel extends CommonModel{
    ...
    public function advertList($params) {
        
        $this->sqlFrom = " tg_advert ";                     //数据库查询表
        ...
        if(!empty($params['status'])){//状态
            $this->sqlWhere .= " and  status=%d ";
            $this->bindValues[] = $params['status'];
        }
        
        if(!empty($params['platform'])){//平台
            $this->sqlWhere .= " and  platform=%d ";
            $this->bindValues[] = $params['platform'];
        }
        ...
        if(!empty($listInfo['data']['list'])){
            foreach($listInfo['data']['list'] as $key=>$val){
                $listInfo['data']['list'][$key]['imgs']= unserialize($val['imgs']);
            }
        }
        return $listInfo;
    ...
    public function advertUpdate($params) {
        $model = M();
        ...
        try {
            ...
            if(!empty($params['id'])){
                $model->table(C('DB_PREFIX') . 'advert')->where(array("id" => $params['id']))->save($params);
            }else{
                $params['ctime'] = time();
                $model->table(C('DB_PREFIX') . 'advert')->add($params);
            }
