...
class GiftModel extends CommonModel{
    ...
    public function giftUpdate($params) {
        $model = M();
        ...
        try {
            ...
            if(!empty($params['id'])){
                $model->table(C('DB_PREFIX') . 'gift')->where(array("id" => $params['id']))->save($params);
            }else{
                $params['ctime'] = time();
                $model->table(C('DB_PREFIX') . 'gift')->add($params);
            }
